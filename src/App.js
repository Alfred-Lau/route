...
// a different context) at the same url, depending on you got there.
//
...
class ModalSwitch extends React.Component {
    ...
    // `/images/2` in a modal, rather than as the main page, keeping the gallery
    // visible behind it.
    //
    // Normally, `/images/2` wouldn't match the gallery at `/`. So, to get both
    // screens to render, we can save the old location and pass it to Switch, so it
    // will think the location is still `/` even though its `/images/2`.
...
const Thumbnail = ({color}) => <div style={{
    width: 50,
    height: 50,
    background: color
}}/>;

const Image = ({color}) => <div style={{
    width: '100%',
    height: 400,
    background: color
}}></div>;
    ...
    <div>
        {IMAGES.map(i => (
            <Link
                key={i.id}
                to={{
                    pathname: `/img/${i.id}`, // this is the trick! 
                    state: { modal: true }
                }}>
